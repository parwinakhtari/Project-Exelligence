...
def add_info(frame, flag, params):
    ...
    cv2.rectangle(frame, (int(0.15 * 640) + 2, 2),
                  (638 - int(0.15 * 640), 75), (135, 135, 88), -1)

    # Adding one more rectangle
    cv2.rectangle(frame, (642 - int(0.15 * 640), 0),
                  (640, 75), (135, 135, 88), -1)
...
def add_feedback(image, params):
    ...
    # Adding rectangle for feedback
    cv2.rectangle(image, (int(0.15 * 640) + 2, 2),
                  (638 - int(0.15 * 640), 75), (135, 135, 88), -1)

    # Adding one more rectangle
    cv2.rectangle(image, (642 - int(0.15 * 640), 0),
                  (640, 75), (135, 135, 88), -1)
...
def plot(params):
    ...
    plt.savefig(params['name'] + '_' + 'graph.png', format="png")
    #plt.show()
...
def elbowFlexion(image, landmarks, params):
    ...
    if angle > 160 and (params['stage'] is None or params['stage'] == 'up'):
        if params['stage'] == 'up':
            params['times'][(params['counter']-1) %
                            params['n_reps']] = abs(time.time() - params['t1'])
            params['rep_time'] = abs(time.time() - params['t1'])
            if abs(params['rep_time'] - params['threshtime']) > 0.2 * params['threshtime']:
                params['error'] += 1
    ...
    if angle < 38 and params['stage'] == 'down':
        ...
        params['counter'] += 1
        params['t2'] = time.time()
        # if params['counter'] == 14 and params['plot'] == False:
        #     plot(params)
        #     params['plot'] = True
...
def squats(image, landmarks, params):
    ...
    if angle < 70 and params['stage'] == 'up':
        ...
        params['counter'] += 1
        params['t2'] = time.time()
        # if params['counter'] == 10 and params['plot'] == False:
        #     plot(params)
        #     params['plot'] = True
        # Error Calculation
